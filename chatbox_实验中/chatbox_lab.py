...
class ChatBox:
    ...
    def chat(self, chat_type):
            ...
            if chat_type == "trader":
                return [
                    {"role": "system", "content": "You are a trader in a cyberpunk world. The user's name is 'Mr.Knight'. "
                                                 ...
                                                 "If your words are longer than 75 words, you need to split them into two or more lines. "
        ...
        input_box = pygame.Rect(10, 520, 790, 32)  # 设置输入框的位置和大小
        color_inactive = pygame.Color('lightskyblue3')  # 输入框未激活时的颜色
        color_active = pygame.Color('dodgerblue2')  # 输入框激活时的颜色
        color = color_inactive  # 初始颜色为未激活颜色
        active = False  # 输入框是否激活
        text = ''  # 输入框中的文本
        done = False  # 是否结束循环
        chat_open = False  # 是否打开聊天界面
        ...
        while not done:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_c:
                        chat_open = not chat_open  # 按下 'c' 键切换聊天界面
                if chat_open:
            ...
            if chat_open:
                ...
                screen.fill((255, 255, 255))  # 设置初始界面背景为白色
                font = pygame.font.Font(None, 36)
                prompt_text = font.render("Press 'c' to open the chat interface", True, (0, 0, 0))
                screen.blit(prompt_text, (100, 280))  # 显示提示信息
